...
echo "send 0 Dummy" > "${APP_HANDLER_STDIN}"
echo "receive 0 Dummy" > "${APP_HANDLER_STDIN}"
...
echo "select 0" > "${APP_HANDLER_STDIN}"
echo "select 0" > "${APP_HANDLER_STDIN}"
echo "send 1 Dummy" > "${APP_HANDLER_STDIN}"
echo "receive 1 Dummy" > "${APP_HANDLER_STDIN}"
echo "Info: Waiting for data to be set..."
sleep 3

# Expected output
APP_EXPECTED_RESULTS_RAW="\033[2J\033[1;1H\033[2J\033[1;1H1970-01-01 01:00:00
Dummy"
APP_EXPECTED_RESULTS=$(echo -e "$APP_EXPECTED_RESULTS_RAW")
APP_ACTUAL_RESULTS=$(<"${APP_STDOUT}")
APP_HANDLER_EXPECTED_RESULTS="select status=0
send status=0
receive status=0
create status=1
select status=1
